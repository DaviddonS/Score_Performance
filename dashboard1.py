...
#setting the page
st.set_page_config(page_title = 'Business Analytics Dashboard', page_icon = 'e:\style.css', layout = 'wide')
...
def metrics():
    ...
    col2.metric(label = "Total Annual Salary", value = f"{df_selection.AnnualSalary.sum():,.0f}", delta = 'Annual Salary Range')
...
def barchart():
            div2= st.empty()
            theme_plotly = None
            fig = px.bar(df_selection, y='AnnualSalary', x='Department', title='Controlled text sizes, positions and angles', text_auto='.2s')
